...
6. For a larger compliant crawl, use the aggressive resumable wrapper:

```bash
python3 scripts/crawl_external_full.py \
  --max-pages-per-source 1000 \
  --max-depth 8 \
  --delay-seconds 0.7 \
  --no-verify-ssl
```

This keeps robots.txt respected by default, skips already-crawled URLs/text hashes, stores raw text under `data/external_raw/`, appends metadata to `data/external_raw/manifest.jsonl`, rebuilds `data/finetune/domain_adaptation.jsonl`, and prints an audit report. Increase `--max-pages-per-source` only when terms/rate limits allow it.
